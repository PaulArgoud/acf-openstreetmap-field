...
class MapProxy extends Singleton {
	...
	public function save_proxy_config( $destination_path ) {
		...
		$wp_filesystem->put_contents(
			untrailingslashit( $destination_path ) . '/acf-osm-proxy-config.json',
			wp_json_encode( $proxy_config )
		);

		// Remove the legacy PHP config (older versions generated executable PHP).
		$legacy = untrailingslashit( $destination_path ) . '/acf-osm-proxy-config.php';
	...
	public function setup_proxy_dir( $force = false ) {
		...
		if ( $force || ! $wp_filesystem->exists( $proxy_path . 'index.php' ) ) {
			...
			$content = '<?php' . "\n";
			$content .= '/* Generously generated by ACF OpenStreetMap Field Plugin */' . "\n";
			$content .= "if ( file_exists( __DIR__ . '/acf-osm-proxy-config.json' ) ) {\n";
			$content .= "\t\$proxy_config = json_decode( file_get_contents( __DIR__ . '/acf-osm-proxy-config.json' ), true );\n";
			$content .= "} elseif ( file_exists( __DIR__ . '/acf-osm-proxy-config.php' ) ) {\n";
		...
		if ( is_multisite() && ( $force || ! $wp_filesystem->exists( $proxy_path . 'acf-osm-proxy-config.json' ) ) ) {
